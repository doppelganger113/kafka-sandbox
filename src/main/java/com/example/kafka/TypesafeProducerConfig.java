...
public final class TypesafeProducerConfig {
    ...
    public Map<String, Object> mapify() {
        ...
        if (!customEntries.isEmpty()) {
            ...
            final var unsupportedKey = customEntries.keySet().stream()
    ...
    private static Set<String> scanClassesForPropertyNames(
        Class<?>... classes
    ) {
        return Arrays.stream(classes)
            ...
            .filter(not(TypesafeProducerConfig::isFieldStringType))
    ...
    private static void tryInsertEntry(
        Map<String, Object> staging, String key, Object value
    ) {
        staging.compute(key, (__key, existingValue) -> {
            ...
            throw new ConflictingPropertyException(
                "Property " + key + " conflicts with an expected property"
            );
        });
    }
}
